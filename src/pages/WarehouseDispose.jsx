...
const WarehouseDispose = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!productId || !quantity) return alert("폐기할 품목과 수량을 입력해주세요.");
    ...
    try {
      ...
      const res = await authAxios.post(`/inventory/dispose`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (res.data.success) {
        alert(user.role === "user" ? "🗑 폐기 요청이 승인 대기로 등록되었습니다." : "🗑 폐기가 즉시 처리되었습니다.");
        setProductId("");
        setProductName("");
        setQuantity("");
        setReason("");
        ...
        <div className="rounded-3xl border border-white/10 bg-white/10 p-8 backdrop-blur">
          <div className="mb-6 text-center">
            ...
            <p className="mt-2 text-sm text-slate-300">
              선택한 창고의 재고에서 폐기할 품목을 지정해 주세요. 승인 후 자동으로 재고가 차감됩니다.
            </p>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-2xl border border-white/10 bg-slate-900/60 px-4 py-3 text-sm text-slate-100 placeholder-slate-500 focus:border-rose-400 focus:outline-none"
                    placeholder="폐기 품목을 검색하세요"
                    disabled={!warehouseId}
                  />
                  {showSuggestions && (
                    <ul className="absolute left-0 right-0 top-full z-20 mt-1 max-h-56 overflow-auto rounded-2xl border border-rose-200 bg-slate-900/95 text-sm shadow-lg">
                      ...
                      {suggestions.map((item, index) => (
                        <li
                          ...
                          className={`cursor-pointer px-4 py-2 ${
                            highlightedIndex === index ? "bg-rose-500/30 text-white" : "text-slate-100 hover:bg-rose-500/20"
                          }`}
                          onMouseDown={(evt) => {
                            evt.preventDefault();
                            handleSelect(item);
